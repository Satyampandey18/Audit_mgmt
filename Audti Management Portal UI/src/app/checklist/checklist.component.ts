...
export class ChecklistComponent implements OnInit {
  ...
  public response: { questionval: string }[] = [];

  ngOnInit(): void {
    if (!this.security.checkLogin()) this.route.navigate(['unauthorisedError']);

    this.loadFlag = false;
    this.response = this.qList.sendResponse();
    for (var str of this.response) {
      str.questionval = ' ';
    }
  }
  ...
  getList(auditVal: number) {
    ...
    let temp: any;
    this.http
      .get(this.queSerUrl + '/' + this.auditType, this.headers)
      .subscribe(
        (response) => {
  ...
  responseYes(i: number) {
    console.log(this.quesList[i]);
    //this.response[i].questionId = this.quesList[i].questionId;
    this.response[i].questionval = 'YES';
    console.log(typeof this.response);
  }

  responseNo(i: number) {
    //this.response[i].questionId = this.quesList[i].questionId;
    this.response[i].questionval = 'NO';
  }
  ...
  getAuditTypeVal() {
    if (this.auditType === 'Internal') return 0;
    else return 1;
  }
}
